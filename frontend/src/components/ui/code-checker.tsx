"use client";

import { useState, ReactNode } from "react";
import { Editor } from "@monaco-editor/react";
import { Button } from "@/components/ui/button";
import { Copy } from "lucide-react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import remarkBreaks from "remark-breaks";
import { Prism as SyntaxHighlighter } from "react-syntax-highlighter";
import { vscDarkPlus } from "react-syntax-highlighter/dist/cjs/styles/prism";

interface CodeCheckerProps {
  code: string;
  setCode: (value: string) => void;
  output: string;
  setOutput: (value: string) => void;
  correctedCode: string;
  setCorrectedCode: (value: string) => void;
}

type CodeProps = {
  inline?: boolean;
  className?: string;
  children?: ReactNode;
};

export default function CodeChecker({
  code,
  setCode,
  output,
  setOutput,
  correctedCode,
  setCorrectedCode,
}: CodeCheckerProps) {
  const [loading, setLoading] = useState(false);

  const handleCheckCode = async () => {
    setLoading(true);

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_KEY}/code-checker`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code }),
      });

      const result = await response.json();
      setLoading(false);

      if (response.ok) {
        setOutput(result.explanation || "No issues found.");
        setCorrectedCode(result.corrected_code || "");
      } else {
        setOutput("Failed to check the code.");
      }
    } catch (error) {
      console.error("Error checking code:", error);
      setLoading(false);
      setOutput("Error occurred while checking code.");
    }
  };

  const handleClearCode = () => setCode("");
  const handleCopyCode = async () => {
    if (correctedCode) {
      await navigator.clipboard.writeText(correctedCode);
      alert("Corrected code copied to clipboard!");
    }
  };

  return (
    <div className="min-h-screen w-full bg-gray-900 text-white rounded flex flex-col">
      {/* Top - Left and Right Panels */}
      <div className="flex flex-1">
        {/* Left - Code Input */}
        <div className="w-1/2 p-4 bg-gray-800 shadow-md rounded-lg m-2 relative">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">Input Your Code Here</h2>
            <Button
              className="bg-gray-700 hover:bg-gray-600 text-white shadow-md rounded"
              onClick={handleClearCode}
            >
              Clear Code
            </Button>
          </div>

          <Editor
            height="85vh"
            value={code}
            onChange={(value) => setCode(value || "")}
            theme="vs-dark"
            className="border border-gray-700 rounded shadow-inner"
          />

          <Button
            onClick={handleCheckCode}
            disabled={loading}
            className="mt-4 w-full bg-blue-700 hover:bg-blue-600 text-white shadow-md rounded"
          >
            {loading ? "Checking..." : "Check Code"}
          </Button>
        </div>

        <div className="w-0.5 bg-gray-600"></div>

        {/* Right - Corrected Code Output */}
        <div className="w-1/2 p-4 bg-gray-800 shadow-md rounded-lg m-2 overflow-y-auto relative">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">Corrected Code</h2>
            <Button
              className="bg-gray-700 hover:bg-gray-600 text-white shadow-md rounded"
              onClick={handleCopyCode}
            >
              <Copy className="mr-2" size={16} />
              Copy Code
            </Button>
          </div>

          {correctedCode && (
            <div className="mt-4 relative bg-gray-700 p-2 rounded-lg shadow-inner">
              <div className="rounded-lg min-h-[55vh] overflow-hidden">
                <Editor
                  value={correctedCode || "// No corrected code provided."}
                  theme="vs-dark"
                  options={{ readOnly: true }}
                  className="border border-gray-600 min-h-[90vh] rounded-lg"
                  height="auto"
                />
              </div>
            </div>
          )}
        </div>
      </div>

      {/* Bottom - Explanation */}
      {output && (
        <div className="w-full p-4 bg-gray-800 shadow-md rounded-lg m-2 mt-4">
          <h2 className="text-xl font-bold">Explanation</h2>

          <div className="mt-4 relative bg-gray-700 p-2 rounded-lg shadow-inner">
            <ReactMarkdown
              className="prose prose-invert mt-2"
              remarkPlugins={[remarkGfm, remarkBreaks]}
              components={{
                code: ({
                  inline,
                  className,
                  children,
                  ...props
                }: CodeProps) => {
                  const match = /language-(\w+)/.exec(className || "");
                  return !inline && match ? (
                    <SyntaxHighlighter
                      style={
                        vscDarkPlus as { [key: string]: React.CSSProperties }
                      }
                      language={match[1]}
                      PreTag="div"
                      {...props}
                    >
                      {String(children)}
                    </SyntaxHighlighter>
                  ) : (
                    <code className={className || ""} {...props}>
                      {children}
                    </code>
                  );
                },
              }}
            >
              {output || "No explanation yet."}
            </ReactMarkdown>
          </div>
        </div>
      )}
    </div>
  );
}
